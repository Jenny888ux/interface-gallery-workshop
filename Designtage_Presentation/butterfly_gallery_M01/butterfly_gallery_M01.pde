...
void setup()
{
  size(1400, 840, P2D); //size (1632, 962);             // OPENGL); // sólo en p5
  //smooth(8); //8
  frameRate(24);
  noCursor();
  ...
  for (int i = 0; i < lines.length; i++) {
    ...
    if (pieces.length == 4) {
      ...
      float x = int(pieces[0]) + w/2 - 120;
      float y = int(pieces[1]) + h/2;

      obstacle[i] = new FBox(w-30, h-20);
...
void draw() {
  ...
  if (apc == countB) {
    ...
    if (obstacleQ.getX() > 0) {              //obstacleQ.getSize()/2) {
      qx -= 2;
      obstacleQ.setPosition(qx, qy);
    } else if (obstacleQ.getX() <= 0 && obstacleQ.getX() > - obstacleQ.getSize()) {
      qx -= .5;
      obstacleQ.setPosition(qx, qy);
    }
    ...
    if (rastro) { 
      pg.smooth(8);// lineas de las bolitas
      pg.beginDraw();
      pg.stroke(255);
      pg.strokeWeight(.2);
  ...
  if (contours.size() > 0) {
    for (Contour contour : contours) {
      ...
      if (contour.getPolygonApproximation().numPoints()> 20 && contour.getPolygonApproximation().area()>2500) {  //(contour.numPoints() > 50) {
